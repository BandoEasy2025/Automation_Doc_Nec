...
Modified to extract documentation requirements solely from website content.
"""
import logging
import re
...
class DocumentationAnalyzer:
    ...
    def merge_grant_data(self, web_data: List[Dict[str, Any]], pdf_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Merges data from web pages into a comprehensive grant overview.
        Modified to function without PDF data.
        
        Args:
            web_data (List[Dict[str, Any]]): Information extracted from web pages.
            pdf_data (List[Dict[str, Any]]): Not used but kept for compatibility.
            
        Returns:
            Dict[str, Any]: Merged grant information.
        """
        if not web_data:
            # Create a minimal structure when no data is available
            return {
    ...
    def generate_documentation_content(self, extracted_docs: Dict[str, List[str]], grant_title: str = "") -> str:
        ...
        Enhanced with categorization and formatting for better readability with bullet points.
        ...
        for category, category_items in categorized_docs.items():
            ...
            for doc_name, content_list in category_items.items():
                ...
                # Add each content item with bullet points
    ...
    def _categorize_documents(self, extracted_docs: Dict[str, List[str]]) -> Dict[str, Dict[str, List[str]]]:
        ...
        """
        categorized = {}
        extracted_docs_copy = extracted_docs.copy()
        
        for category_name, category_docs in self.document_categories.items():
            categorized[category_name] = {}
            
            for doc_name, doc_items in extracted_docs_copy.items():
                ...
                if doc_name in category_docs or any(cat_doc in doc_name.lower() for cat_doc in category_docs):
                    ...
                    # Mark as categorized by removing from extracted_docs_copy
                    extracted_docs_copy.pop(doc_name, None)
        
        # Add "Other" category for remaining items
        if extracted_docs_copy:
            categorized["Altro"] = extracted_docs_copy
    ...
    def generate_summary(self, grant_data: Dict[str, Any]) -> str:
        ...
        Enhanced to provide more structured and readable output with bullet points.
