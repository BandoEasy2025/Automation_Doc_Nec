...
class DatabaseManager:
    ...
    def get_active_grants(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table(config.BANDI_TABLE) \
                .select("id, link_bando, link_sito_bando, documentazione_necessaria, nome_bando") \
                .eq("stato", "Attivo") \
                .execute()
    ...
    def get_all_grants(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table(config.BANDI_TABLE) \
                .select("id, link_bando, link_sito_bando, documentazione_necessaria, nome_bando, stato") \
                .execute()
    ...
    def update_documentation(self, grant_id: str, documentation: str) -> bool:
        """
        Updates the documentazione_necessaria column and update_in_db timestamp for a specific grant.
        ...
        try:
            ...
            # Update both documentation and update_in_db timestamp
            response = self.supabase.table(config.BANDI_TABLE) \
                .update({
                    "documentazione_necessaria": documentation,
                    "updated_at": current_time,  # Regular timestamp
                    "update_in_db": current_time  # Special tracking timestamp
                }) \
                .eq("id", grant_id) \
                .execute()
            
            if hasattr(response, 'data') and len(response.data) > 0:
                logger.info(f"Successfully updated documentation and timestamp for grant {grant_id}")
                return True
            else:
